...
Spark is used to run data-processing algorithms across a cluster. Normally, you would connect to an external cluster. However, 
you will connect to your local machine as if it were a cluster. This will allow you to familiarise yourself with spark and sparklyr.

The code chunk below creates a spark connection to your machine as if it were a cluster, and then loads the mtcars dataset
into a Spark dataframe. mtcars is a dataset that is built into R that we will use as an example.
